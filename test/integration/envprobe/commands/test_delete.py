...
from envprobe.commands.delete import command
...
def test_list(capfd, args):
    ...
    assert(not stdout)
    assert(stderr.rstrip() == "No snapshot named 'NONEXISTENT'.")
    ...
    stdout, stderr = capfd.readouterr()
    assert(not stdout)
    assert(stderr.rstrip() == "No snapshot named 'test1'.")
    ...
    args.SNAPSHOT = "Foo/bar"
    command(args)
    stdout, stderr = capfd.readouterr()
    assert(not stdout)
    assert(not stderr)

    args.SNAPSHOT = "Bar/thing"
    command(args)
    stdout, stderr = capfd.readouterr()
    assert(not stdout)
    assert(not stderr)

    args.SNAPSHOT = "Bar/Baz/x"
    command(args)
    stdout, stderr = capfd.readouterr()
    assert(not stdout)
    assert(not stderr)
