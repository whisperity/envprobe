...
def test_envprobe_loaded(sh, tmp_path):
    ...
    retcode, _ = sh.execute_command("envprobe --help", timeout=2)
    assert(not retcode)
...
def test_get_variable(sh):
    retcode, _ = sh.execute_command("envprobe get", timeout=1)
    assert(retcode == 2)
...
def test_add_and_remove(sh, tmp_path):
    ...
    retcode, _ = sh.execute_command("echo 'echo \"Hello\"' > dummy/exe",
                                    timeout=1)
    assert(not retcode)
    retcode, _ = sh.execute_command("chmod +x dummy/exe", timeout=1)
    assert(not retcode)
    ...
    retcode, result = sh.execute_command("ep +PATH dummy", timeout=1)
    assert(not retcode)
    assert(not result)
    retcode, result = sh.execute_command("which exe", timeout=1)
    assert(not retcode)
